...
from utils.app_components import app_details, delete_app, edit_app, export_dsl
...
def get_apps():
    ...
    try:
        ...
        for app in st.session_state.apps_cache:
            ...
            if (
                st.session_state.filter_mode != "all"
                and app["mode"] != st.session_state.filter_mode
            ):
                continue
...
def import_dsl_dialog():
    ...
    with col2:
        if st.button(
            "导入", key="confirm_import", type="primary", use_container_width=True
        ):
            # 获取DSL内容
            ...
            try:
                ...
                with st.spinner("正在导入DSL..."):
                    if import_option == "更新现有应用" and app_id_to_update:
                        result = client.import_app_dsl(
                            import_dsl_content, app_id_to_update
                        )
                    else:
...
def batch_export_dsl_dialog():
    ...
    with col1:
        if st.button(
            "导出为ZIP格式", key="btn_confirm_export_zip", use_container_width=True
        ):
            try:
                client = DifyClient.get_connection()
    ...
    with col2:
        if st.button(
            "导出为JSON格式", key="btn_confirm_export_json", use_container_width=True
        ):
            try:
                client = DifyClient.get_connection()
                progress_bar = st.progress(0)
...
def batch_rename_dialog():
    ...
    st.subheader("修改选项")
    edit_mode = st.radio(
        "修改模式", ["添加前缀", "添加后缀", "查找替换", "重命名模式"], horizontal=True
    )
    ...
    if edit_mode == "添加前缀":
        ...
        preview = [
            name.replace(find_text, replace_text) if find_text else name
        ...
        for i, name in enumerate(app_names):
            if pattern:
                new_name = pattern.replace("{index}", str(i + 1)).replace(
                    "{name}", name
                )
        ...
        if st.button(
            ...
            if not any(a != b for a, b in zip(app_names, preview)):
                ...
                with st.spinner("正在批量修改应用名称..."):
                    for i, (app_id, old_name, new_name) in enumerate(
                        zip(app_ids, app_names, preview)
                    ):
                        try:
                            # 获取应用当前详情以保留描述
                            app_details = client.fetch_app(app_id)
...
def batch_delete_dialog():
    ...
    confirm_text = st.text_input(
        "请输入 'DELETE' 以确认删除", key="batch_delete_confirm"
    )
        ...
        try:
            ...
            if all_tags:
                ...
                selected_tag = st.sidebar.selectbox(
                    ...
                    index=(
                        0
                        if st.session_state.filter_tag is None
                        else list(tag_options.keys()).index(st.session_state.filter_tag)
                    ),
                )
...
def show_batch_operations_panel():
    ...
    with col3:
        if st.button(
            "批量删除", key="btn_batch_delete", type="primary", use_container_width=True
        ):
...
def show_app_list():
    ...
    primary_actions = [
        {
            ...
            "on_click": lambda: setattr(
                st.session_state, "show_create_app_dialog", True
            ),
        },
        {
            "label": "导入应用DSL",
            "key": "btn_import_dsl",
            "on_click": lambda: setattr(
                st.session_state, "show_import_dsl_dialog", True
            ),
        },
        {
            "label": "刷新列表",
    ...
    columns = [
        ...
        {
            "field": lambda app: format_timestamp(app.get("updated_at", 0)),
            "title": "更新时间",
        },
    ]
    ...
    selected_df = data_display(
        apps, columns, key="app_table", multi_select=st.session_state.multi_select_mode
    )
...
def show_app_details():
    ...
    app_details(
        app_id,
        on_back=lambda: set_sesstion_state("show_app_details", False, rerun=True),
    )
