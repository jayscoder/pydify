...
from utils.ui_components import (
    page_header, app_selector, loading_spinner, error_placeholder, success_message,
...
def get_workflow_tool(app_id):
    ...
    # 使用缓存
    if st.session_state.workflow_tool_cache is None or st.session_state.current_workflow_app_id != app_id:
        try:
            with loading_spinner("加载工作流工具详情..."):
                st.session_state.workflow_tool_cache = client.fetch_workflow_tool(app_id)
...
def view_provider_details():
    """查看工具提供者详情"""
    if st.session_state.selected_provider is not None and not st.session_state.selected_provider.empty:
        provider_id = st.session_state.selected_provider.iloc[0]["_id"]
        provider = next((p for p in get_tool_providers() if p['id'] == provider_id), None)
        if provider:
...
def view_workflow_tool():
    """查看工作流工具详情"""
    if st.session_state.selected_workflow_app is not None and not st.session_state.selected_workflow_app.empty:
...
def show_tool_provider_list():
    ...
    # 主要功能切换按钮区
    actions = [
        {"label": "管理工作流工具", "key": "btn_workflow_tools", "on_click": view_workflow_apps},
        {"label": "刷新列表", "key": "btn_refresh_providers", "on_click": lambda: reset_cache() or st.rerun()}
    ]
    action_bar(actions)
    ...
    # 创建筛选类型的复选框
    provider_types = sorted(list(set(provider.get('type', 'unknown') for provider in providers)))
    selected_types = st.multiselect(
        "筛选工具提供者类型",
        options=provider_types,
        default=provider_types
    )
    
    # 筛选工具提供者
    filtered_providers = [p for p in providers if p.get('type', 'unknown') in selected_types]
    ...
    if not selected_df.empty:
        ...
        # 操作按钮
        provider_actions = [
            {"label": "查看详情", "key": "btn_view_provider", "on_click": view_provider_details}
        ]
...
def show_workflow_apps():
    ...
    actions = [
        ...
        {"label": "刷新列表", "key": "btn_refresh_workflow", "on_click": lambda: reset_cache('workflow_app') or st.rerun()}
    ]
    action_bar(actions)
    ...
    if not selected_df.empty:
        ...
        if app:
            ...
            if app.get('published_as_tool', False):
                app_actions = [
                    {"label": "查看工具详情", "key": "btn_view_tool", "on_click": view_workflow_tool},
                    {"label": "编辑工具", "key": "btn_edit_tool", "on_click": edit_workflow_tool}
                ]
            else:
                app_actions = [
                    {"label": "发布为工具", "key": "btn_publish_tool", "on_click": lambda: publish_workflow_app(app_id)}
                ]
...
def show_workflow_tool_details():
    ...
    # 获取应用信息
    app = next((a for a in get_workflow_apps() if a['id'] == st.session_state.current_workflow_app_id), None)
    if not app:
        st.error("获取应用信息失败")
    ...
    # 操作按钮
    actions = [
        {"label": "返回列表", "key": "btn_back_tool", "on_click": back_to_workflow_apps},
        {"label": "编辑工具", "key": "btn_edit_from_view", "on_click": edit_workflow_tool}
    ]
    action_bar(actions)
    ...
    with col2:
        ...
        st.write(f"**应用类型**: 工作流")
        st.write(f"**发布状态**: {'已发布' if app.get('published_as_tool', False) else '未发布'}")
...
def show_edit_workflow_tool():
    ...
    with st.form("编辑工作流工具"):
        ...
        qualified_name = st.text_input("工具标识", value=tool_data.get('qualified_name', ''))
        ...
        if submitted:
            ...
            try:
                ...
                with loading_spinner("正在更新工作流工具..."):
                    client.update_workflow_tool(st.session_state.current_workflow_app_id, update_data)
