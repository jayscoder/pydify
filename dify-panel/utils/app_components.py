...
def app_details(app_id, on_back=None):
    ...
    try:
        ...
        # 应用基本信息选项卡
        tabs = st.tabs(
            ["基本详情", "详情JSON", "API密钥", "标签管理", "DSL可视化", "工具展示"]
        )
        ...
        with tabs[0]:
            ...
            with col1:
                ...
                app_url = client.app_url(
                    app_id=app_details["id"], app_mode=app_details["mode"]
                )
                st.write(f"**应用链接**: {app_url}")
            with col2:
                st.write(
                    f"**创建时间**: {format_timestamp(app_details.get('created_at', 0))}"
                )
                st.write(
                    f"**更新时间**: {format_timestamp(app_details.get('updated_at', 0))}"
                )
            ...
            with col1:
                ...
                st.write(
                    f"**启用网站**: {'是' if app_details.get('enable_site', False) else '否'}"
                )
            with col2:
                st.write(
                    f"**启用API**: {'是' if app_details.get('enable_api', False) else '否'}"
                )
                st.write(f"**API基础URL**: {app_details.get('api_base_url', '无')}")
                st.write(
                    f"**使用图标作为回答图标**: {'是' if app_details.get('use_icon_as_answer_icon', False) else '否'}"
                )
            ...
            with st.expander("删除API密钥"):
                api_key_to_delete = st.selectbox(
                    ...
                    format_func=lambda x: next(
                        (k["token"] for k in api_keys if k["id"] == x), x
                    ),
                )
        ...
        with st.expander("添加标签"):
            ...
            if available_tags:
                tag_to_add = st.selectbox(
                    ...
                    format_func=lambda x: next(
                        (t["name"] for t in available_tags if t["id"] == x), x
                    ),
                )
        ...
        if app_tags:
            with st.expander("移除标签"):
                tag_to_remove = st.selectbox(
                    ...
                    format_func=lambda x: next(
                        (t["name"] for t in app_tags if t["id"] == x), x
                    ),
                )
...
def edit_app_dialog(app_id, app_info=None):
    ...
    with st.form("edit_app_form"):
        ...
        app_description = st.text_area(
            "应用描述", value=app_info.get("description", "")
        )
...
def delete_app(app_id, on_success=None):
    ...
    if st.session_state.confirm_delete_app:
        with st.container():
            ...
            with col2:
                if st.button(
                    "确认删除", key=f"proceed_delete_{app_id}", type="primary"
                ):
                    client = DifyClient.get_connection()
                    try:
