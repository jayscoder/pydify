...
class ChatflowClient(DifyBaseClient):
    ...
    def audio_to_text(self, file_path: str, user: str) -> Dict[str, Any]:
        ...
        with open(file_path, 'rb') as file:
            ...
            response = self._request("POST", 'audio-to-text', headers=headers, files=files, data=data)
            return response.json()
            
    def audio_to_text_obj(self, file_obj: BinaryIO, filename: str, user: str) -> Dict[str, Any]:
        """
        使用文件对象进行语音转文字。
