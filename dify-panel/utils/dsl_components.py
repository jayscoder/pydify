...
def dsl_graph(dsl_content):
    ...
    try:
        ...
        if (
            not dsl_content
            or "workflow" not in dsl_content
            or "graph" not in dsl_content["workflow"]
        ):
        ...
        for node in dsl_nodes:
            ...
            # 创建节点
            nodes.append(
                Node(id=node_id, label=node_title, color=color, shape=shape, size=25)
            )
        ...
        for edge in dsl_edges:
            ...
            edges.append(
                Edge(source=source_id, target=target_id, label=label, type=curve_type)
            )
        ...
        nodes_df = (
            ...
            if nodes_data
            else pd.DataFrame(columns=["ID", "类型", "标题", "位置X", "位置Y"])
        )
        edges_df = (
            pd.DataFrame(edges_data)
            if edges_data
            else pd.DataFrame(
                columns=["源节点", "目标节点", "源类型", "目标类型", "条件"]
            )
        )
        ...
        with tab1:
            ...
            with col1:
                st.markdown(
                    f"<div style='background-color: {node_types['start']}; padding: 10px; border-radius: 5px; margin: 2px;'>开始节点</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f"<div style='background-color: {node_types['end']}; padding: 10px; border-radius: 5px; margin: 2px;'>结束节点</div>",
                    unsafe_allow_html=True,
                )

            with col2:
                st.markdown(
                    f"<div style='background-color: {node_types['llm']}; padding: 10px; border-radius: 5px; margin: 2px;'>LLM节点</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f"<div style='background-color: {node_types['code']}; padding: 10px; border-radius: 5px; margin: 2px;'>代码节点</div>",
                    unsafe_allow_html=True,
                )

            with col3:
                st.markdown(
                    f"<div style='background-color: {node_types['template-transform']}; padding: 10px; border-radius: 5px; margin: 2px;'>模板转换节点</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f"<div style='background-color: {node_types['if-else']}; padding: 10px; border-radius: 5px; margin: 2px;'>条件分支节点</div>",
                    unsafe_allow_html=True,
                )

            with col4:
                st.markdown(
                    f"<div style='background-color: {node_types['tool']}; padding: 10px; border-radius: 5px; margin: 2px;'>工具节点</div>",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f"<div style='background-color: {node_types['custom']}; padding: 10px; border-radius: 5px; margin: 2px;'>自定义节点</div>",
                    unsafe_allow_html=True,
                )
            ...
            if not nodes_df.empty:
                if search_node:
                    filtered_nodes_df = nodes_df[
                        nodes_df.apply(
                            lambda row: search_node.lower() in str(row).lower(), axis=1
                        )
                    ]
                ...
                if node_ids:  # 只有当列表非空时才显示选择框
                    selected_node = st.selectbox(
                        "选择节点查看详情", node_ids, key="select_node"
                    )
                    if selected_node:
                        st.subheader(f"节点 '{selected_node}' 详情")
                        node_detail = next(
                            (node for node in dsl_nodes if node["id"] == selected_node),
                            None,
                        )
                        if node_detail:
                            st.json(node_detail)
            ...
            if not edges_df.empty:
                if search_edge:
                    filtered_edges_df = edges_df[
                        edges_df.apply(
                            lambda row: search_edge.lower() in str(row).lower(), axis=1
                        )
                    ]
                ...
                if source_nodes:  # 只有当列表非空时才显示选择框
                    selected_edge_source = st.selectbox(
                        "源节点", source_nodes, key="select_source"
                    )
                    if selected_edge_source:
                        filtered_targets = edges_df[
                            edges_df["源节点"] == selected_edge_source
                        ]["目标节点"].tolist()
                        if filtered_targets:  # 确保目标节点列表非空
                            selected_edge_target = st.selectbox(
                                "目标节点", filtered_targets, key="select_target"
                            )
                            if selected_edge_target:
                                st.subheader(
                                    f"连接 '{selected_edge_source}' -> '{selected_edge_target}' 详情"
                                )
                                edge_detail = next(
                                    (
                                        edge
                                        ...
                                        and edge["target"] == selected_edge_target
                                    ),
                                    None,
                                )
                                if edge_detail:
