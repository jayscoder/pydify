    ...
    if app_id not in st.session_state.api_keys_cache:
        try:
            with loading_spinner("加载API密钥..."):
                st.session_state.api_keys_cache[app_id] = client.fetch_app_api_keys(app_id)
        except Exception as e:
...
def delete_api_key():
    """确认删除API密钥"""
    if st.session_state.selected_api_key is not None and not st.session_state.selected_api_key.empty:
...
def show_api_key_list():
    ...
    if st.session_state.current_app_id:
        app_name = next((app['name'] for app in apps if app['id'] == st.session_state.current_app_id), "未知应用")
        
        st.subheader(f"应用 '{app_name}' 的API密钥")
        
        # 添加操作按钮
        actions = [
            {"label": "创建API密钥", "key": "btn_create_api_key", "on_click": create_api_key},
            {"label": "刷新列表", "key": "btn_refresh_api_keys", "on_click": lambda: reset_cache(st.session_state.current_app_id) or st.rerun()}
        ]
        action_bar(actions)
        ...
        if not selected_df.empty:
            ...
            # 操作按钮
            key_actions = [
                {"label": "删除API密钥", "key": "btn_delete_selected", "color": "danger", "on_click": delete_api_key}
            ]
...
def show_delete_api_key():
    ...
    # 获取API密钥信息
    api_keys = get_api_keys(st.session_state.current_app_id)
    api_key = next((k for k in api_keys if k['id'] == st.session_state.current_api_key_id), None)
    ...
    confirmation_dialog(
        ...
        on_confirm=lambda: perform_delete(st.session_state.current_app_id, st.session_state.current_api_key_id),
