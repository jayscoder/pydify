...
# 导入工具类
from utils.dify_client import DifyClient, DifyAppMode
from utils.ui_components import (
    page_header, confirmation_dialog, 
...
if 'app_view_mode' not in st.session_state:
    st.session_state.app_view_mode = 'list'  # 可选值: list, create, view, edit, delete, dsl_export, dsl_import
...
def get_app_details(app_id):
    ...
    # 检查缓存
    if (st.session_state.app_details_cache is not None and 
        'id' in st.session_state.app_details_cache and 
        st.session_state.app_details_cache['id'] == app_id):
...
def view_app():
    """查看应用详情"""
    if st.session_state.selected_app is not None and not st.session_state.selected_app.empty:
...
def show_app_list():
    ...
    actions = [
        ...
        {"label": "刷新列表", "key": "btn_refresh_apps", "on_click": lambda: reset_cache() or st.rerun()}
    ]
    action_bar(actions)
    ...
    if not selected_df.empty:
        ...
        app_actions = [
            ...
            {"label": "API密钥", "key": "btn_api_keys_selected", "on_click": manage_api_keys},
            {"label": "标签管理", "key": "btn_tags_selected", "on_click": manage_tags},
            {"label": "导出DSL", "key": "btn_export_dsl_selected", "on_click": export_dsl},
            {"label": "删除应用", "key": "btn_delete_selected", "color": "danger", "on_click": delete_app}
        ]
...
def show_app_details():
    ...
    st.divider()
    detail_actions = [
        {"label": "编辑应用", "key": "btn_edit_from_detail", "on_click": lambda: edit_app()},
        {"label": "导出DSL", "key": "btn_export_dsl_from_detail", "on_click": lambda: export_dsl()},
        {"label": "删除应用", "key": "btn_delete_from_detail", "color": "danger", "on_click": lambda: delete_app()}
    ]
...
def show_create_app():
    ...
    with st.form("创建应用"):
        ...
        mode = st.selectbox("应用类型", options=list(mode_options.keys()), format_func=lambda x: mode_options[x])
        ...
        if submitted:
            ...
            try:
                ...
                with loading_spinner("正在更新应用..."):
                    updated_app = client.update_app(st.session_state.current_app_id, name, description)
...
def show_dsl_import():
    ...
    if import_option == "更新现有应用":
        ...
        selected_app_name = st.selectbox("选择要更新的应用", options=list(app_options.keys()))
