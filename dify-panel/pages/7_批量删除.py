...
def main():
    ...
    # label: 输入框的标签提示文字
    # help: 输入框下方的帮助提示文字
    delete_ids_input = st.text_area("请输入要删除的应用ID", help="每行一个ID，或用逗号分隔多个ID。")
        ...
        if st.button("开始删除", type="primary"): # "type='primary'" 使按钮更醒目
            ...
            if len(delete_ids) > 0:
                ...
                for index, app_id in enumerate(delete_ids):
                    ...
                    progress_bar.progress(progress_value, text=f"正在删除: {app_id} ({index + 1}/{len(delete_ids)})")
                    time.sleep(0.1) # 短暂暂停，让用户能看清进度更新，实际API调用可能不需要
                ...
                if failed_deletions:
                    ...
                    for item in failed_deletions:
                        st.markdown(f"- **ID:** `{item['id']}` - **原因:** {item['reason']}")
