...
def edit_tag():
    """编辑标签"""
    if (
        st.session_state.selected_tag is not None
        and not st.session_state.selected_tag.empty
    ):
...
def show_tag_list():
    ...
    actions = [
        {"label": "创建新标签", "key": "btn_create_tag", "on_click": create_tag},
        {
            "label": "应用标签关联",
            "key": "btn_tag_binding",
            "on_click": bind_tag_to_app,
        },
        {
            "label": "刷新列表",
            "key": "btn_refresh_tags",
            ...
            try:
                ...
                with loading_spinner("正在更新标签..."):
                    updated_tag = client.update_tag(
                        st.session_state.current_tag_id, name
                    )
...
def show_delete_tag():
    ...
    # 获取标签名称
    tag = next(
        (t for t in get_tags() if t["id"] == st.session_state.current_tag_id), None
    )
    if not tag:
        st.error("找不到选定的标签")
        back_to_list()
